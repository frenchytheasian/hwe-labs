...
reviews = reviews.withColumn("review_timestamp", lit(datetime.now()))
...
product_category_df = reviews.select("product_category")
product_category_df.show(50)
# Digital_video_Games appears to be the most common
...
reviews.createOrReplaceTempView("reviews")
most_helpful_review = spark.sql("SELECT product_title, helpful_votes FROM reviews ORDER BY helpful_votes DESC LIMIT 1")
most_helpful_review.show(truncate=False)
# product_title: Xbox Live Subscription, helpful_votes: 993
...
int_cols = spark.sql("SELECT CAST(star_rating as INT) star_rating, CAST(helpful_votes as INT) helpful_votes, CAST(total_votes as INT) total_votes FROM reviews")
int_cols.show(10)

# Question 10: Find the date with the most purchases.
# Print the date and total count of the date which had the most purchases.
spark.sql("SELECT purchase_date, count(*) as count FROM reviews GROUP BY purchase_date ORDER BY purchase_date DESC LIMIT 1").show(truncate=False)
# 2015-08-31, 108
...
##Use overwrite mode.
reviews.write.mode("overwrite").json("week2_sql/output")
